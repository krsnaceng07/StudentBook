      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingHorizontal: 24 }}>
          <View className="items-center mb-10">
            <View className="h-20 w-20 bg-blue-600 rounded-2xl items-center justify-center mb-6 shadow-lg shadow-blue-600/20">
              <Text className="text-white font-bold text-3xl">🎓</Text>
            </View>
            <Text className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Student Login</Text>
            <Text className={`mt-2 text-base ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Welcome back to your builder world</Text>
          </View>
          ...
          <View className="space-y-4">
            ...
            <View className={`rounded-xl border px-4 py-3 mb-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
              <Text className={`text-xs uppercase mb-1 font-semibold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Email</Text>
              <TextInput
            ...
            <TouchableOpacity onPress={handleLogin} disabled={isLoading} className="bg-blue-600 rounded-xl py-4 mt-6 items-center shadow-lg shadow-blue-600/30">
              {isLoading ? <ActivityIndicator color="white" /> : <Text className="text-white font-bold text-lg">Login</Text>}
            </TouchableOpacity>

            <TouchableOpacity onPress={() => router.push('/(auth)/student/signup')} className="mt-6">
              <Text className={`text-center text-base ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                New student? <Text className="text-blue-500 font-bold">Sign Up</Text>
              </Text>
            </TouchableOpacity>
